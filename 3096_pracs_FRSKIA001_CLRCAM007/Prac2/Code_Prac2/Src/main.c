...
int main(void)
{
    ...
    for (uint8_t i = 0; i < 6; i++) write_to_address(i, patterns[i]);
    ...
    while (1)
    {
        ...
        if ((HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin) == GPIO_PIN_RESET) && (Button0Pushed == 0b0)) {
            Button0Pushed = 0b1;
            if (TIM16->ARR == 500-1) TIM16->ARR = 1000-1;
            else TIM16->ARR = 500-1;
        }

        if (HAL_GPIO_ReadPin(Button0_GPIO_Port, Button0_Pin) == GPIO_PIN_SET) Button0Pushed = 0b0;
    }
    /* USER CODE END 3 */
}
